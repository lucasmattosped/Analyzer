...
import { Users, Clock, BookOpen, TrendingUp, AlertCircle, Download, FileSpreadsheet } from 'lucide-react';
...
export function ClassDashboard() {
  const { classAnalysis, students, selectedClass } = useAppStore();

  // Generate PDF for collective dashboard
  const generatePDF = async () => {
    ...
    doc.text('BLUEBERRY MATH — DASHBOARD COLETIVO', 105, yPos, { align: 'center' });
    yPos += 15;

    // Subtitle
    doc.setFontSize(11);
    doc.setFont('helvetica', 'normal');
    const className = selectedClass?.name || classAnalysis.class_name || '';
    const subtitle = `${className} | Período: ${classAnalysis.period}`;
    ...
    doc.setFontSize(10);
    doc.setFont('helvetica', 'normal');
    doc.text(`Alunos: ${classAnalysis.total_students}`, 20, yPos);
    yPos += 7;
    doc.text(`Média de atividades: ${classAnalysis.metrics.avg_activities.toFixed(2)}`, 20, yPos);
    yPos += 7;
    doc.text(`Média de tempo: ${classAnalysis.metrics.avg_time_minutes.toFixed(1)} min`, 20, yPos);
    yPos += 7;
    doc.text(`Adesão: ${classAnalysis.metrics.adherence_percent.toFixed(1)}%`, 20, yPos);
    yPos += 15;
    ...
    doc.setFontSize(10);
    doc.setFont('helvetica', 'normal');
    doc.text(`Verde: ${classAnalysis.traffic_light.green_percent.toFixed(1)}%`, 20, yPos);
    yPos += 7;
    doc.text(`Amarelo: ${classAnalysis.traffic_light.yellow_percent.toFixed(1)}%`, 20, yPos);
    yPos += 7;
    doc.text(`Vermelho: ${classAnalysis.traffic_light.red_percent.toFixed(1)}%`, 20, yPos);
    yPos += 15;
    ...
    doc.setFontSize(10);
    doc.setFont('helvetica', 'normal');
    doc.text(classAnalysis.recommendation, 20, yPos);

    const filename = `Dashboard_Coletivo_${className.replace(/\s+/g, '_')}.pdf`;
    doc.save(filename);
  };

  // Generate Excel for collective dashboard
  const generateExcel = async () => {
    if (!classAnalysis) return;

    const className = selectedClass?.name || classAnalysis.class_name || '';
    const worksheetData = [
      ['BLUEBERRY MATH — DASHBOARD COLETIVO'],
      [`${className} | Período: ${classAnalysis.period}`],
      [],
      ['MÉTRICAS GERAIS'],
      [`Total de Alunos: ${classAnalysis.total_students}`],
      [`Média de Atividades: ${classAnalysis.metrics.avg_activities.toFixed(2)}`],
      [`Média de Tempo (min): ${classAnalysis.metrics.avg_time_minutes.toFixed(1)}`],
      [`Adesão (%): ${classAnalysis.metrics.adherence_percent.toFixed(1)}%`],
      [],
      ['DISTRIBUIÇÃO POR STATUS'],
      [`Verde: ${classAnalysis.traffic_light.green_count} alunos (${classAnalysis.traffic_light.green_percent.toFixed(1)}%)`],
      [`Amarelo: ${classAnalysis.traffic_light.yellow_count} alunos (${classAnalysis.traffic_light.yellow_percent.toFixed(1)}%)`],
      [`Vermelho: ${classAnalysis.traffic_light.red_count} alunos (${classAnalysis.traffic_light.red_percent.toFixed(1)}%)`],
      [],
      ['RECOMENDAÇÃO'],
      [classAnalysis.recommendation],
      [],
      ['DIFICULDADES CRÍTICAS'],
      ...classAnalysis.critical_difficulties.map((d, i) => [`Top ${i + 1}: ${d}`])
    ];
    ...
    const filename = `Dashboard_Coletivo_${className.replace(/\s+/g, '_')}.xlsx`;
        ...
        <CardHeader>
          <div className="flex items-start justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <ExportButtons
                data={{
                  className: classAnalysis.class_name,
                  period: classAnalysis.period,
                  totalStudents: classAnalysis.total_students,
                  avgActivities: classAnalysis.metrics.avg_activities,
                  avgTime: classAnalysis.metrics.avg_time_minutes,
                  adherence: classAnalysis.metrics.adherence_percent,
                  trafficLight: classAnalysis.traffic_light,
                  recommendation: classAnalysis.recommendation,
                  criticalDifficulties: classAnalysis.critical_difficulties
                }}
                fileNamePrefix={`Dashboard_Coletivo_${classAnalysis.class_name.replace(/\s+/g, '_')}`}
