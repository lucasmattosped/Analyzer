 ...
 *
 * Fetches student data for a specific class from the official Blueberry Math API
...
export async function POST(request: NextRequest) {
  try {
    const { token, cookies, classGuid, days } = await request.json();
...
/**
 * Recursively searches for a users array in the response object
 */
